...
// 中序线索化处理
void in_thread(treeNode_t *tree, treeNode_t **pre) // 把指针当成一个变量，要想在全局中可以保存改变的值，就要用二级指针，故此处 pre 用二级指针
{
    if (tree)
    {
        in_thread(tree->lchild, pre); // 递归左子树
        ...
        *pre = tree; // 更新前驱节点pre为当前节点
        ...
        in_thread(tree->rchild, pre); // 递归右子树
    }
}
...
{
    while (tree && tree->lchild && tree->l_tag == 0)
        tree = tree->lchild;
    return tree;
}
...
int main(int argc, char *argv[])
{
    ...
    in_thread(T, &pre); // 进行中序线索化

    if (pre)
    {
        pre->r_tag = 1; // 中序遍历完pre到了最后一个节点，最后一个节点的后继线索指向NULL
        pre->rchild = NULL;
    }

    // 使用线索进行中序遍历
    printf("In-order traversal using threading: ");
