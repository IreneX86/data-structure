...
// 递归创建二叉树
void create_tree(treeNode_t **tree, treeNode_t *parent)
{
    char ch;
    scanf(" %c", &ch);
    ...
    else
    {
        ...
        */
        ...
        (*tree)->r_tag = 0;
        (*tree)->parent = parent;
        create_tree(&((*tree)->lchild), *tree);
        create_tree(&((*tree)->rchild), *tree);
    }
}
...
// 后序线索化处理
void post_thread(treeNode_t *tree, treeNode_t **pre)
{
    if (tree)
    {
        // 通过检查 l_tag 和 r_tag 确定是否对子树进行递归。这样可以防止对线索指针进行递归，从而保护程序不会访问非法内存。
        if (tree->l_tag == 0)               // 当有左子树时
            post_thread(tree->lchild, pre); // 递归左子树
        if (tree->r_tag == 0)               // 当有左子树时
            post_thread(tree->rchild, pre); // 递归右子树
...
// 获取后序遍历的第一个节点：最左下的叶子节点
treeNode_t *get_first(treeNode_t *tree)
{
    while (tree)
    {
        if (tree->l_tag == 0 && tree->lchild)
            tree = tree->lchild;
        else if (tree->r_tag == 0 && tree->rchild)
            tree = tree->rchild;
        else
            break;
    }
...
treeNode_t *get_next(treeNode_t *tree)
{
    ...
    else
    {
        ...
        if (!tree->parent)
            return NULL;
        // 若为右子树
        else if (tree->parent->rchild == tree)
            return tree->parent;
        // 若为左子树
        else
        {
            if (tree->parent->l_tag == 0)
                return get_first(tree->parent->rchild);
            else
                return tree->parent;
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    post_thread(T, &pre); // 进行后序线索化

    // 使用线索进行后序遍历
    printf("Post-order traversal using threading: ");
    for (treeNode_t *node = get_first(T); node != NULL; node = get_next(node))
    {
        printf("%c ", node->data);
    }
    printf("\n");
